    ...
    , ui(new Ui::detailedinfo)
{
    ...
    connect(sp,SIGNAL(from_sp(QString,QString)),this,SLOT(receive_from_sp(QString,QString)));
}
...
void detailedinfo::stress(int n )
{
    QTableWidgetItem *myitem = ui->tableWidget->item(n, 0);
    ui->tableWidget->scrollToItem(myitem, QAbstractItemView::PositionAtCenter);
    ui->tableWidget->setCurrentItem(myitem);
}
...
void detailedinfo::on_printButton_clicked()
{
    ...
    {
        QMessageBox::critical(this, tr("警告"), tr("失败"));
    }
}


void detailedinfo::on_pushButton_clicked()
{
    sp->show();
    return;
}
